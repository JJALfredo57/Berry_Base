...
class OrderController extends Controller
{
    ...
    private function acknowledgeCustomCod(string $coId, object $co, object $order, float $depositAmount, bool $isFullPayment, int $uid)
    {
        ...
        DB::table('order_tracking')->insert([
            ...
            'status'     => 'Confirmed',
            'notes'      => $isFullPayment
                ? "COD full payment †‚‚‚‚‚‚PHP {$depositAmount} acknowledged. Order auto-confirmed."
                : "COD deposit †‚‚‚‚‚‚‚PHP {$depositAmount} acknowledged. Order auto-confirmed. Remaining: †‚‚‚‚‚‚" . ($order->total_price - $depositAmount),
            'created_at' => now(),
        ]);
    ...
    private function sendCustomToKitchen(object $co, object $order)
    {
        ...
        $payInfo     = $order->payment_method === 'COD'
            ? CakeshopHelper::shortPaymentCode($order->payment_method, $order->fulfillment_type ?? null) . " †‚‚…‚‚‚‚‚ Deposit †‚‚‚‚‚‚PHP {$order->deposit_amount} acknowledged"
            : "GCash Deposit †‚‚‚‚‚‚‚PHP {$order->deposit_amount} †‚‚…‚‚‚‚ Paid";
