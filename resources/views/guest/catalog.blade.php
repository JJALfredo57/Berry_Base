...
<div class="catalog-grid" id="catalogGrid">
    ...
    <div class="catalog-item"
      ...
      <div class="catalog-card card h-100" style="{{ !$isAvailable ? 'opacity:.75' : '' }};transition:transform .3s cubic-bezier(.34,1.56,.64,1),box-shadow .3s ease">
        ...
        {{-- Image --}}
        <div class="img-zoom-wrap position-relative overflow-hidden" style="border-radius:1.1rem 1.1rem 0 0;height:260px">
        ...
        <div class="card-body d-flex flex-column p-4">
          ...
          <div class="d-flex align-items-center gap-1 mb-2">
            @for($i=1;$i<=5;$i++)
              <i class="bi bi-star{{ $i <= round($avgRating) ? '-fill' : '' }}" style="color:#fbbf24;font-size:clamp(.74rem,1.5vw,.8rem)"></i>
            @endfor
          ...
          <div class="mb-3">
            ...
            <div class="d-flex flex-wrap gap-1">
              @foreach($sizes as $sz)
              <span class="badge" style="background:var(--primary-light);color:var(--primary);font-size:clamp(.68rem,1.3vw,.72rem)">
          ...
          <div class="d-flex align-items-center justify-content-between">
            <div>
              @if(!empty($pricing['has_discount']))
                <div class="text-muted text-decoration-line-through" style="font-size:clamp(.72rem,1.35vw,.78rem)">₱{{ number_format($pricing['original_unit_price'],2) }}</div>
                <span class="fw-bold fs-5" style="color:#dc2626">₱{{ number_format($pricing['final_unit_price'],2) }}</span>
                <div style="font-size:clamp(.66rem,1.2vw,.7rem);color:#be123c;font-weight:700">{{ $pricing['badge_text'] }}</div>
              @else
                <span class="fw-bold fs-5" style="color:var(--primary)">₱{{ number_format($p->price,2) }}</span>
              @endif
              @if(count($sizes) > 0)
                <div class="text-muted" style="font-size:clamp(.68rem,1.3vw,.72rem)">Base price</div>
              @endif
            </div>
            @if($p->total_sold > 0)
            <span class="badge rounded-pill" style="background:#fff1f2;color:#be123c;font-size:.72rem">
