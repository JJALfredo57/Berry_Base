...
<div>
  ...
  <div style="background:#fff;border-radius:var(--radius-lg);border:1.5px solid var(--gray-100);padding:1rem 1.25rem;margin-bottom:1.25rem">
    ...
    <div id="sellerOrderFilterSummary" style="font-size:.78rem;color:var(--gray-500);margin-top:.6rem">Showing {{ $orders->firstItem() ?? 0 }}ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ{{ $orders->lastItem() ?? 0 }} of {{ $orders->total() }} orders</div>
  </div>
  ...
  <div class="seller-order-item {{ $needsPickupAction ? 'action-needed' : '' }}"
    ...
    <div style="padding:1rem 1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;background:{{ $needsPickupAction ? '#fff7fb' : (in_array($o->status,['Pending','Pending Review']) ? '#FFFBF5' : '#fff') }}">
      ...
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:.5rem;flex-wrap:wrap;margin-bottom:.25rem">
          ...
          @else
            <span style="background:#ecfdf5;color:#047857;font-size:.68rem;font-weight:700;padding:.2rem .5rem;border-radius:99px">Ready-made</span>
          @endif
        </div>
        ...
        @if($groupOrders->isNotEmpty())
          <div class="mt-1 small" style="color:#0369a1;font-weight:700">
      ...
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:.4rem;flex-shrink:0">
        <div style="font-size:1rem;font-weight:700;color:var(--primary)">ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->total_price ?? 0),2) }}</div>
    ...
    <div id="order-detail-{{ $o->id }}" style="display:none">
      ...
      <div style="margin-bottom:1rem;background:#FCE4EC;border-radius:10px;padding:.75rem 1rem;display:flex;align-items:center;gap:.75rem">
        ...
        <div>
          ...
          <div style="font-size:.8rem;color:#880E4F;margin-top:.1rem">
            Customer must pay ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->deposit_amount ?? 0), 2) }} deposit via GCash before this order enters the queue.
          </div>
        </div>
      </div>
      @elseif($o->deposit_required && ($o->deposit_status ?? '') === 'paid')
      <div style="margin-bottom:1rem;background:#E8F5E9;border-radius:10px;padding:.75rem 1rem;display:flex;align-items:center;gap:.75rem">
        ...
        <div>
          ...
          <div style="font-size:.8rem;color:#2E7D32;margin-top:.1rem">
            ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->deposit_amount ?? 0), 2) }} deposit paid. Remaining ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->total_price ?? 0) - (float)($o->deposit_amount ?? 0), 2) }} to be collected on {{ $o->fulfillment_type === 'Delivery' ? 'delivery' : 'pickup' }}.
          </div>
        </div>
      </div>
      @endif
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem 1.25rem;font-size:.82rem">
        ...
        <div>
          ...
          <strong>{{ $o->order_customer_phone ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â' }}</strong>
        ...
        @if(count($items) > 1)
          <div style="grid-column:1/-1">
            ...
            @foreach($items as $item)
              <div class="small">ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ <strong>{{ $item->product_name }}</strong> x{{ $item->quantity }} @if($item->selected_size) ({{ $item->selected_size }}) @endif</div>
            @endforeach
          </div>
        @endif
        <div><span style="color:var(--gray-500)">Qty / Size</span><br><strong>{{ $o->quantity ?? 1 }}x {{ $o->selected_size ?? ($custom->size_label ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â') }}</strong></div>
        <div><span style="color:var(--gray-500)">Fulfillment</span><br><strong>{{ $o->fulfillment_type ?? 'Pickup' }}</strong></div>
        <div><span style="color:var(--gray-500)">Schedule</span><br><strong>{{ $o->schedule_date ? \Carbon\Carbon::parse($o->schedule_date)->format('M d, Y') : 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â' }}{{ $o->schedule_time ? ' '.$o->schedule_time : '' }}</strong></div>
        @if($o->fulfillment_type === 'Delivery')
        <div style="grid-column:1/-1"><span style="color:var(--gray-500)">Delivery Address</span><br><strong>{{ $o->delivery_address ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â' }}</strong></div>
        @endif
        <div><span style="color:var(--gray-500)">Payment</span><br><strong>{{ $o->payment_method ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â' }}</strong></div>
        ...
        <div><span style="color:var(--gray-500)">Total</span><br><strong style="color:var(--primary)">ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->total_price ?? 0),2) }}</strong></div>
        @if($o->delivery_fee)
        <div><span style="color:var(--gray-500)">Delivery Fee</span><br><strong>ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->delivery_fee ?? 0),2) }}</strong></div>
        @endif
        ...
        <div><span style="color:var(--gray-500)">Deposit</span><br>
          <strong style="color:{{ $isCancelledOrder ? '#991b1b' : (($o->deposit_status??'') === 'paid' ? '#16a34a' : '#880E4F') }}">
            ...
            @else
              ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->deposit_amount ?? 0),2) }} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â {{ ($o->deposit_status??'') === 'paid' ? 'Paid' : 'Pending' }}
            @endif
          </strong>
        </div>
        @endif
      ...
      <div style="margin-top:.85rem">
        ...
        @foreach($addons as $a)
        <div style="font-size:.81rem;color:var(--gray-700)">ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ {{ $a->addon_name ?? 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â' }} @if($a->addon_price ?? 0) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($a->addon_price ?? 0),2) }} @endif</div>
        @endforeach
      </div>
      @endif
      ...
      <div style="margin-top:.65rem;background:{{ $refund->status === 'pending' ? '#fff7ed' : ($refund->status === 'refunded' ? '#ecfdf5' : '#f8fafc') }};border-radius:8px;padding:.75rem .9rem;font-size:.81rem;color:#111827;border:1px solid {{ $refund->status === 'pending' ? '#fed7aa' : ($refund->status === 'refunded' ? '#bbf7d0' : '#e5e7eb') }}">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap">
          <div>
            ...
            <div><strong>Refund amount:</strong> ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)$refund->refund_amount, 2) }}</div>
      ...
      <div style="display:flex;align-items:flex-start;gap:.75rem;flex-wrap:wrap">
        ...
        <div style="flex:1;min-width:260px">
          ...
          <div style="font-size:.82rem;color:#92400e;line-height:1.5;font-weight:600">
            Customer paid ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)$refund->payment_amount_paid, 2) }}. Approve only after sending the GCash refund and uploading the receipt.
          </div>
        </div>
      </div>
      <div class="row g-3 mt-1">
    ...
    @if($o->status === 'Pickup')
      ...
      <div data-pickup-payment-wait="{{ $o->id }}" style="border-top:1px solid var(--gray-100);padding:.9rem 1.25rem;background:linear-gradient(135deg,#fffbeb 0%,#f8fafc 100%);display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
        <div style="display:flex;align-items:flex-start;gap:.75rem;flex:1;min-width:260px">
          ...
          <div>
            ...
            <div style="font-size:.82rem;color:#d97706;line-height:1.5;font-weight:600">
              ...
              @if($o->payment_method === 'GCash') (GCash ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->total_price ?? 0) - (float)($o->deposit_amount ?? 0), 2) }} remaining)@endif.
        ...
        <span style="background:#FEF3C7;color:#92400E;border:1.5px solid #FDE68A;border-radius:var(--radius-md);padding:.35rem .875rem;font-size:.78rem;font-weight:700;margin-left:auto">
          <i class="bi bi-lock"></i> Locked ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Unpaid
        </span>
      </div>
      @endif
    @endif

    {{-- Awaiting Deposit ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â seller info only, no actions until paid --}}
      ...
      <div style="display:flex;align-items:flex-start;gap:.75rem;flex:1;min-width:260px">
        ...
        <div>
          ...
          <div style="font-size:.82rem;color:#880E4F;line-height:1.5;font-weight:600">
            Customer needs to pay the 50% deposit (ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â±{{ number_format((float)($o->deposit_amount ?? 0),2) }}) via GCash. The order will appear in your queue once payment is confirmed.
          </div>
        </div>
      </div>
    ...
    <div style="border-top:1px solid var(--gray-100);padding:.9rem 1.25rem;background:linear-gradient(135deg,#fff8fb 0%,#f8fafc 100%);display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
      ...
      @elseif($isRegularOrder && $o->status === 'Confirmed' && ($o->fulfillment_type ?? 'Pickup') === 'Pickup')
        <form action="{{ route('seller.orders.status', $o->id) }}" method="POST" class="m-0">
          @csrf
          <input type="hidden" name="status" value="Pickup">
          <button type="submit" class="btn btn-success btn-sm fw-semibold"
                  data-cs-confirm="Mark this ready-made order as ready for pickup?"
                  data-cs-title="Ready for Pickup"
      ...
      @elseif(($o->status ?? '') === 'Ready for Rider' && ($o->fulfillment_type ?? 'Pickup') === 'Delivery')
        <form action="{{ route('seller.orders.assign_rider', $o->id) }}" method="POST" class="d-flex gap-2 flex-wrap align-items-center m-0">
          ...
          @if($riders->isEmpty())
            <span class="small text-danger fw-semibold">No active riders.</span>
          @endif
        </form>
      @elseif($isCustomOrder && $o->status === 'Pending Review')
